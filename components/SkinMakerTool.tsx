...
export default function SkinMakerTool({ embedded = false }: { embedded?: boolean }) {
  ...
  const snapRectsToWidgetGrid = () => {
    ...
    setRects(prev => {
      ...
      const filled = [
        ...sorted,
        ...Array.from({ length: Math.max(0, GRID_COUNT - sorted.length) }, (_, k) => ({
          id: Date.now() + k,
          x: 0, y: 0, w: WIDGET_W, h: ch,
          url: 'https://', labelColor: '#ffffff', borderColor: '',
        } as LinkRect)),
      ];
  ...
  const downloadSkin = async () => {
    ...
    for (const r of rects) {
      ...
      if (r.label) {
        ...
        ctx.textBaseline = 'top';
        ctx.fillText(r.label, r.x + r.w / 2, r.y + Math.round(r.h * LABEL_Y_RATIO));
      }
    }
                ...
                {selectedEl && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button onClick={() => centerElV(selectedEl.id)}
                        className="flex items-center justify-center gap-1.5 py-2 rounded-lg border border-gray-300 bg-white text-xs font-semibold text-gray-600 hover:border-gray-500 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-500">
                        <AlignHorizontalJustifyCenter size={13} style={{ transform: 'rotate(90deg)' }} /> 세로 가운데
                      </button>
                    </div>
                  </div>
                )}
              ...
              <>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2 gap-1">
                    <label className="block text-sm font-medium text-gray-700 flex-shrink-0">링크 ({rects.length})</label>
                    <div className="flex items-center gap-1">
                      <button onClick={snapRectsToWidgetGrid}
              ...
              >
                ...
                {rects.map(r => (
                  <div key={r.id} style={{
                    ...
                    {r.label && (
                      <span style={{ position: 'absolute', left: 0, right: 0, top: Math.round(r.h * LABEL_Y_RATIO), display: 'flex', justifyContent: 'center', fontSize: 20, fontWeight: 'bold', color: r.labelColor || '#333333', pointerEvents: 'none', userSelect: 'none' }}>{r.label}</span>
                    )}
                  </div>
                ))}
